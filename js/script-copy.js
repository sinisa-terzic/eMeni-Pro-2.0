document.addEventListener('DOMContentLoaded', () => {
    ...
    /* ***** INICIJALIZACIJA I OSNOVNE FUNKCIJE ***** */
    // Funkcije za pokretanje aplikacije, učitavanje podataka i postavljanje osnovnih elemenata.
    ...
    /* ***** END INICIJALIZACIJA I OSNOVNE FUNKCIJE ***** */

    /* ***** SLIDER I NAVIGACIJA ***** */
    // Funkcije za upravljanje slajderom, automatski pregled i navigaciju.
    ...
    // Postavljanje event listenera za slider i navigaciju
    function setupEventListeners() {
        ...
        // Dodavanje touch eventova za navigaciju
        addTouchEvents(elements.container);
    }

    /* ***** END SLIDER I NAVIGACIJA ***** */
    ...
    /* ***** KORPA ***** */
    // Funkcije za upravljanje korpom, dodavanje/uklanjanje stavki i prikaz korpe.
    ...
    // Funkcije za pretragu stavki i filtriranje po kategorijama.

    // Provera da li su svi potrebni elementi za pretragu pronađeni u DOM-u
    ...
    let currentFilter = ''; // Trenutno aktivni filter (kategorija)

    // Funkcija za generisanje radio dugmadi za filtere
    function generateFilterRadioButtons() {
        const uniqueCategories = new Set(); // Koristimo Set da bismo izbegli duplikate

        // Prolazimo kroz sve kategorije i stavke da bismo prikupili jedinstvene kategorije
        state.categories.forEach(category => {
            category.translations.forEach(item => {
                if (item.details) {
                    uniqueCategories.add(item.details); // Dodajemo kategoriju u Set
                }
            });
        });

        // Za svaku jedinstvenu kategoriju kreiramo radio dugme i labelu
        uniqueCategories.forEach(category => {
            const radioButton = document.createElement('input');
            radioButton.type = 'radio';
            radioButton.name = 'categoryFilter'; // Grupišemo radio dugmad po imenu
            radioButton.value = category;
            radioButton.id = `filter-${category}`; // Jedinstveni ID za svako dugme

            const label = document.createElement('label');
            label.htmlFor = `filter-${category}`; // Povezujemo labelu sa radio dugmetom
            label.textContent = category;

            // Dodajemo radio dugme i labelu u kontejner za filtere
            elements.filterContainer.appendChild(radioButton);
            elements.filterContainer.appendChild(label);
            elements.filterContainer.appendChild(document.createElement('br')); // Novi red između dugmadi

            // Event listener za klik na radio dugme
            radioButton.addEventListener('click', (e) => {
                if (radioButton.checked && currentFilter === category) {
                    // Ako je radio dugme već čekirano i kliknuto ponovo, poništavamo filter
                    radioButton.checked = false;
                    currentFilter = '';
                    elements.searchInput.value = ''; // Brišemo unos u polje za pretragu
                    elements.searchResults.innerHTML = ''; // Brišemo rezultate pretrage
                } else {
                    // Postavljamo trenutni filter i pokrećemo pretragu
            ...
            // Event listener za promenu stanja radio dugmeta
            radioButton.addEventListener('change', () => {
                if (radioButton.checked) {
                    currentFilter = category;
                    elements.searchInput.value = category;
                    searchItem(category); // Pokrećemo pretragu kada se filter promeni
                }
            });
        });
    }
    ...
    // Event listener za klik van polja za pretragu i rezultata
    ...
    /* ***** END PRETRAGA I FILTRIRANJE ***** */

    /* ***** PADAJUĆI MENI I SETTINGS ***** */
    // Funkcije za upravljanje padajućim menijem i settings menijem.
    ...
    // Event listeneri za padajući meni
    ...
    /* ***** END PADAJUĆI MENI I SETTINGS ***** */

    /* ***** OPCIJA POZIVANJA (CALL MENU) ***** */
    // Funkcije za upravljanje opcijom pozivanja (call menu).
    ...
    /* ***** END OPCIJA POZIVANJA (CALL MENU) ***** */

    /* ***** POPUP PROZOR ***** */
    // Funkcije za prikaz i upravljanje popup prozorom sa detaljima stavke.
...
`;
    ...
    /* ***** LOCALSTORAGE I STANJE APLIKACIJE ***** */
    // Funkcije za čuvanje i učitavanje stanja aplikacije u localStorage.
    ...
    // Funkcija za vraćanje čekiranih checkboxova iz localStorage
    function restoreCheckedCheckboxes() {
        state.selectedItems.forEach(item => {
            const checkbox = document.querySelector(`.item-checkbox[data-id="${item.id}"]`);
            if (checkbox) {
    ...
    // Funkcija za vraćanje količina iz localStorage
    function restoreQuantities() {
        state.selectedItems.forEach(item => {
            const quantityElement = document.querySelector(`.menu-card[data-id="${item.id}"] .quantity`);
            const priceElement = document.querySelector(`.menu-card[data-id="${item.id}"] .price`);
    ...
    // Funkcija za čuvanje istorije u localStorage
    ...
    // Funkcija za učitavanje istorije iz localStorage
    ...
    /* ***** END LOCALSTORAGE I STANJE APLIKACIJE ***** */

    /* ***** POMOĆNE FUNKCIJE ***** */
    // Funkcije za optimizaciju, debounce, lenjo učitavanje i druge pomoćne operacije.
    ...
    // Debounce funkcije za otvaranje menija
    ...
    // Funkcija za primenu aktivnih klasa
    ...
    // Funkcija za izračunavanje visine dataContainer-a
    ...
    // Funkcija za resetovanje količina na 1
    ...
    /* ***** END POMOĆNE FUNKCIJE ***** */

    /* ***** EVENT LISTENERI ***** */
    // Postavljanje event listenera za interakciju sa korisnikom.
    ...
    function setupEventListeners() {
        ...
        elements.searchBtn.addEventListener('click', () => toggleSearch('open'));
        document.getElementById('searchTitle').addEventListener('click', () => toggleSearch('close'));
    ...
    /* ***** TOUCH I KEYBOARD INTERAKCIJE ***** */
    // Funkcije za podršku touch i keyboard interakcija.
    ...
    /* ***** END TOUCH I KEYBOARD INTERAKCIJE ***** */

    /* ***** RESIZE I LAZY LOADING ***** */
    // Funkcije za upravljanje promenama veličine prozora i lenjo učitavanje slika.
    ...
    /* ***** END RESIZE I LAZY LOADING ***** */

    /* ***** HISTORIJA I PRAĆENJE ***** */
    // Funkcije za praćenje istorije pregledanih slajdova i stavki.
    ...
    /* ***** END HISTORIJA I PRAĆENJE ***** */

    /* ***** ANIMACIJE I TRANSITION ***** */
    // Funkcije za upravljanje animacijama i tranzicijama.

    // Funkcija za animaciju uklanjanja stavke iz korpe
    function removeItem(id) {
        const checkbox = document.querySelector(`.menu-card[data-id="${id}"] .item-checkbox`);
        const itemElement = document.querySelector(`.selected-item[data-id="${id}"]`);
    ...
    // Funkcija za animaciju highlight-a prilikom pretrage
    function scrollToSubcategoryTitle(title) {
        // Pronalazimo odgovarajući naslov u DOM-u
        const targetElement = Array.from(document.querySelectorAll('.subcategory-title'))
            .find(el => el.textContent.trim().toLowerCase() === title.toLowerCase());
    ...
    // Funkcija za animaciju highlight-a prilikom pretrage stavki
    function displaySearchResults(results) {
        elements.searchResults.innerHTML = ''; // Brišemo prethodne rezultate
    ...
    /* ***** END ANIMACIJE I TRANSITION ***** */

    /* ***** ERROR HANDLING I DEBUGGING ***** */
    // Funkcije za rukovanje greškama i debugiranje.

    // Funkcija za prikazivanje grešaka prilikom učitavanja podataka
    ...
    // Funkcija za prikazivanje grešaka prilikom kreiranja sekcije sa podacima
    ...
    // Funkcija za prikazivanje grešaka prilikom učitavanja korpe iz localStorage
    function loadCartFromLocalStorage() {
        const cart = localStorage.getItem('cart');
        if (cart) {
            try {
                ...
                renderEmptyCart(); // Prikaži praznu korpu ako se desi greška
            }
        } else {
            // Ako nema stavki u localStorage, prikaži praznu korpu
            renderEmptyCart();
    ...
    // Funkcija za prikazivanje grešaka prilikom učitavanja istorije iz localStorage
    function loadHistoryFromLocalStorage() {
        const history = localStorage.getItem('history');
        if (history) {
            try {
                ...
                console.error('Greška pri učitavanju istorije iz localStorage:', error);
            }
        }
    }

    // Funkcija za prikazivanje grešaka prilikom učitavanja stanja aplikacije iz localStorage
    function loadAppStateFromLocalStorage() {
        const savedState = localStorage.getItem('appState');
        if (savedState) {
            try {
                ...
                console.error('Greška pri učitavanju stanja aplikacije iz localStorage:', error);
            }
        }
    }

    /* ***** END ERROR HANDLING I DEBUGGING ***** */

    // Debounce funkcija za ograničenje broja poziva pretrage tokom unosa
    const debouncedSearch = debounce(() => {
        const query = document.getElementById('searchInput').value;
        if (query) {
            searchItem(query); // Pokrećemo pretragu ako postoji upit
        }
    });

    // Event listener za unos u polje za pretragu
    document.getElementById('searchInput').addEventListener('input', debouncedSearch);
